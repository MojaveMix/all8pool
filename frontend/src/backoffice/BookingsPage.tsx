...
const BookingsPage = () => {
  ...
  const [showPlayerModal, setShowPlayerModal] = useState<{ bookingId: string; playerIndex: 1 | 2 } | null>(null);
  ...
  const assignPlayer = async (userId: string | null, name: string | null) => {
    ...
    try {
      ...
      await api.patch(`/bookings/${showPlayerModal.bookingId}/players`, payload);
  ...
  const startMatch = async (booking: Booking) => {
    try {
      await api.post('/matches', {
        ...
        poolHallId: hallId,
        player1Id: booking.id, // Placeholder if booking.userId is null
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm font-bold uppercase tracking-widest mt-1">Manage player bookings and schedules</p>
        </div>
      ...
      <div className="bg-secondary rounded-[2.5rem] border border-gray-800 overflow-hidden">
        ...
        <div className="p-8 border-b border-gray-800 flex justify-between items-center bg-secondary/50">
          <div className="flex items-center gap-6">
            ...
            <div className="flex gap-2">
              <button onClick={() => setCurrentDate(subWeeks(currentDate, 1))} className="p-2 hover:bg-primary rounded-xl transition-colors border border-gray-800">
                <ChevronLeft size={18} />
              </button>
              <button onClick={() => setCurrentDate(new Date())} className="px-4 py-2 hover:bg-primary rounded-xl text-[10px] font-black uppercase border border-gray-800">Today</button>
              <button onClick={() => setCurrentDate(addWeeks(currentDate, 1))} className="p-2 hover:bg-primary rounded-xl transition-colors border border-gray-800">
        ...
        {view === 'calendar' ? (
          <div className="grid grid-cols-7 divide-x divide-gray-800 border-b border-gray-800">
            {weekDays.map(day => (
              <div key={day.toString()} className={`p-6 min-h-[400px] ${isSameDay(day, new Date()) ? 'bg-accent/5' : ''}`}>
                <div className="text-center mb-6">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{format(day, 'EEE')}</p>
                  <p className={`text-2xl font-black mt-1 ${isSameDay(day, new Date()) ? 'text-accent' : 'text-white'}`}>{format(day, 'd')}</p>
                </div>
                <div className="space-y-3">
                  {bookings.filter(b => isSameDay(new Date(b.startTime), day)).map(booking => (
                    <div key={booking.id} className="p-3 bg-primary rounded-xl border border-gray-800 text-[10px] font-bold group hover:border-accent transition-colors cursor-pointer">
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-gray-400">{format(new Date(booking.startTime), 'HH:mm')}</span>
                        <div className={`w-2 h-2 rounded-full ${booking.status === 'confirmed' ? 'bg-success' : 'bg-warning animate-pulse'}`} />
                      </div>
                      <p className="text-white truncate">{booking.player1Name} {booking.player2Name ? `vs ${booking.player2Name}` : ''}</p>
                      <p className="text-accent mt-1 tracking-tighter italic">Table #{booking.table.number}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-800/50">
                {bookings.map(booking => (
                  <tr key={booking.id} className="hover:bg-primary/40 transition-colors group">
                    <td className="px-10 py-6">
                      <div className="flex items-center gap-4">
                        <div className="flex flex-col gap-1">
                           <div className="flex items-center gap-2">
                              <span className="font-bold text-white">{booking.player1Name}</span>
                              <button onClick={() => setShowPlayerModal({ bookingId: booking.id, playerIndex: 1 })} className="p-1 hover:text-accent text-gray-600 transition-colors">
                                <Plus size={14} />
                              </button>
                           </div>
                           <div className="flex items-center gap-2">
                              {booking.player2Name ? (
                                <span className="text-xs text-gray-400">vs {booking.player2Name}</span>
                              ) : (
                                <span className="text-[10px] text-gray-600 italic">No opponent</span>
                              )}
                              <button onClick={() => setShowPlayerModal({ bookingId: booking.id, playerIndex: 2 })} className="p-1 hover:text-accent text-gray-600 transition-colors">
                                <Plus size={14} />
                              </button>
                           </div>
                        </div>
                    ...
                    <td className="px-10 py-6">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-white">{format(new Date(booking.startTime), 'MMM d, yyyy')}</span>
                        <span className="text-xs text-gray-500 font-medium">{format(new Date(booking.startTime), 'HH:mm')} - {format(new Date(booking.endTime), 'HH:mm')}</span>
                      </div>
                    </td>
                    <td className="px-10 py-6 font-black italic text-accent tracking-tighter text-lg">#{booking.table.number}</td>
                    <td className="px-10 py-6 text-sm text-gray-400 font-bold">${booking.totalPrice}</td>
                    <td className="px-10 py-6">
                      <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border ${
                        booking.status === 'confirmed' ? 'bg-success/10 text-success border-success/20' : 'bg-warning/10 text-warning border-warning/20 animate-pulse'
                      }`}>
                        {booking.status}
                      </span>
                    </td>
                    <td className="px-10 py-6">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button className="p-3 bg-danger/10 text-danger rounded-xl hover:bg-danger hover:text-primary transition-all" title="Cancel">
                          <X size={18} />
                        </button>
                      </div>
                    </td>
          ...
          <div className="bg-secondary w-full max-w-md p-10 rounded-[2.5rem] border border-gray-800 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="space-y-8">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-[10px] uppercase font-black tracking-widest"><span className="bg-secondary px-4 text-gray-500">OR DYNAMIC GUEST</span></div>
              </div>
